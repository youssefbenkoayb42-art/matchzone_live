...
export function buildOpenFootballForm(matches) {
  const form = {};

  const finished = (matches || [])
    .filter((match) => hasFinalScore(match))
    .sort(
      (a, b) =>
        new Date(b.fixture.date).getTime() -
        new Date(a.fixture.date).getTime()
  ...
  for (const match of finished) {
    ...
    if (!form[homeName]) form[homeName] = [];
    if (!form[awayName]) form[awayName] = [];

    if (form[homeName].length < 5) {
      form[homeName].push({
        result:
          homeScore > awayScore
            ? "W"
            : homeScore < awayScore
              ? "L"
              : "D",
        opponent: awayName || "الخصم",
        score: homeScore + "-" + awayScore,
      });
    }

    if (form[awayName].length < 5) {
      form[awayName].push({
        result:
          awayScore > homeScore
            ? "W"
