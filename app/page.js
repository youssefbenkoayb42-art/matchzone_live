  ...
  return (
    <main style={{ background: "radial-gradient(circle at top, #0f2b1d 0%, #06100b 100%)", minHeight: "100vh", color: "#fff", padding: "25px 15px", fontFamily: "system-ui, -apple-system, sans-serif", direction: "rtl" }}>
      ...
      <section id="news-section">
        <h2 style={{ fontSize: "22px", fontWeight: "800", marginBottom: "25px" }}>📰 غيوم الأخبار والنبض الرياضي</h2>
        {loadingNews ? ( <p style={{ color: "#7f8c8d" }}>جاري مزامنة وكالات الأنباء الرياضية...</p> ) : (
          <>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: "25px" }}>
              {news.slice(0, visibleCount).map((article, index) => (
                <article key={index} style={{ background: "rgba(255,255,255,0.01)", borderRadius: "20px", overflow: "hidden", display: "flex", flexDirection: "column", border: "1px solid rgba(255,255,255,0.04)" }}>
                  {article.urlToImage && ( <div style={{ width: "100%", height: "160px" }}><img src={article.urlToImage} alt={article.title} style={{ width: "100%", height: "100%", objectFit: "cover" }} /></div> )}
                  <div style={{ padding: "20px", flex: 1, display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                    <div>
                      <h4 style={{ margin: "0 0 10px 0", fontSize: "15px", lineHeight: "1.5" }}>{article.title}</h4>
                      <p style={{ fontSize: "13px", color: "#7f8c8d", margin: "0 0 20px 0" }}>{article.description?.slice(0, 90)}...</p>
                    </div>
                    <a href={article.url} target="_blank" rel="noopener noreferrer" style={{ color: "#2ecc71", textDecoration: "none", fontSize: "13px", fontWeight: "bold" }}> تحليل الخبر بالكامل ← </a>
                  </div>
                </article>
              ))}
            </div>
            {visibleCount < news.length && (
              <div style={{ textAlign: "center", marginTop: "40px" }}>
                <button onClick={loadMoreNews} style={{ background: "rgba(255,255,255,0.03)", color: "#fff", border: "1px solid rgba(255,255,255,0.08)", padding: "12px 35px", borderRadius: "30px", cursor: "pointer", fontSize: "14px", fontWeight: "bold" }}> 🔄 تحميل طوفان الأخبار الإضافية </button>
              </div>
            )}
          </>
        )}
