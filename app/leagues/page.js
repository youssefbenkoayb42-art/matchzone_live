          ...
          <div style={styles.grid}>
            {LEAGUES.map(([slug, name, englishName, logo]) => (
              <Link key={slug} href={"/leagues/" + slug} style={styles.card}>
                ...
                <span style={styles.cardLink}>استكشف البطولة <span>←</span></span>
              </Link>
            ))}
          </div>
        </section>
...
const styles = {
  ...
  card: { minHeight: 260, display: "flex", flexDirection: "column", padding: 18, borderRadius: 24, background: "linear-gradient(145deg,#10251c,#0a1511)", border: "1px solid #1e3d30", color: "#f4f8f6", textDecoration: "none", boxShadow: "0 12px 35px rgba(0,0,0,.16)", transition: "transform .2s ease, border-color .2s ease" },
  ...
  cardLink: { color: "#2ecc71", fontWeight: 900, fontSize: 12, marginTop: 16 },
